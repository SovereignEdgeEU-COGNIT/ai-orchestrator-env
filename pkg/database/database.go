...
func (db *Database) createMetricsTable() error {
	sqlStatement := `CREATE TABLE ` + db.dbPrefix + `METRICS (ID TEXT, TYPE INTEGER, TS TIMESTAMPTZ, CPU DOUBLE PRECISION, MEMORY DOUBLE PRECISION, DISK_READ DOUBLE PRECISION, DISK_WRITE DOUBLE PRECISION, NET_RX DOUBLE PRECISION, NET_TX DOUBLE PRECISION)`
	_, err := db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
